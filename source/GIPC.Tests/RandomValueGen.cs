...
namespace GIPC.Tests
{
    public class RandomValueGen
    {
        ...
        public static T GetRandomFrom<T>(IEnumerable<T> selection)
        {
            var min = 0;
            var max = selection.Count() - 1;
            return selection.Skip(GetRandomInt(min, max)).FirstOrDefault();
        }
        ...
        public static string GetRandomAlphaString(int minLength = 1, int maxLength = 32)
        {
